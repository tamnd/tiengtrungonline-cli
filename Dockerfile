...
RUN apk add --no-cache ca-certificates tzdata \
 && adduser -D -H -u 10001 tiengtrungonline

COPY $TARGETPLATFORM/tiengtrungonline /usr/bin/tiengtrungonline

USER tiengtrungonline

ENTRYPOINT ["/usr/bin/tiengtrungonline"]
