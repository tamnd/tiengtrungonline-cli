...
describes what users get. Pick a format with `-o`, or let tiengtrungonline choose:
a table when writing to a terminal, JSONL when piped.

## Formats

```bash
tiengtrungonline <command> -o table   # aligned columns for reading
tiengtrungonline <command> -o jsonl   # one JSON object per line, for piping
tiengtrungonline <command> -o json    # a single JSON array
tiengtrungonline <command> -o csv     # spreadsheet friendly
tiengtrungonline <command> -o tsv     # tab-separated
tiengtrungonline <command> -o url     # just the URL column
tiengtrungonline <command> -o raw     # the underlying bytes, unformatted
```
...
```bash
tiengtrungonline <command> --fields id,title,url
```
...
```bash
tiengtrungonline <command> --template '{{.URL}} {{.Title}}'
```
...
```bash
tiengtrungonline <command>            # a table, because this is a terminal
tiengtrungonline <command> | wc -l    # JSONL, because this is a pipe
```
